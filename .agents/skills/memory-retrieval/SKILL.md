...
由 AGENTS.md/CLAUDE.md 硬规则 #5 指引。AI 判断需要项目历史上下文时**主动调用**，不做会话开头的无条件触发。
...
| 写论文某节 | "§4.5 消融实验结果、PPO vs SAC 对比数据" |
| 调研背景 | "待读文献、文献库状态、近期调研" |

## 置信度标注

引用 `.pipeline/survey/`、`.pipeline/contracts/`、`.pipeline/experiments/` 或 `bigmemory/冷区/调研记录/` 时，先看文件开头 frontmatter：
