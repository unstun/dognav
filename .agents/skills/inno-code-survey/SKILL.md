...
## Navigation Project Override

For `machine-dog-nav`, the active Trellis task or Dr Sun's request replaces the
legacy `selected_idea` input. Store candidate clones below a dated topic
directory such as `references/upstream/2026-07-23_waypoint_navigation/<repo>`.

Before treating a repository as a candidate, record:

- canonical URL and license;
- default branch and pinned commit;
- supported robot, simulator, ROS version, sensors, and compute assumptions;
- original installation and demo command;
- whether the repository was only inspected or actually reproduced.

Do not copy source files into project implementation paths during the survey.
Selection, reproduction, and Lite3 integration are separate tasks.
...
| `references/upstream/<repo_name>/` | Cloned GitHub repositories |
| `references/upstream/model_survey.md` | Code survey implementation report |
| `references/upstream/logs/` | Phase A & B agent cache files |
...
Read `selected_idea` and identify 2–3 **missing technical components** — novel or specialized parts that are likely NOT in the standard repos already present in `references/upstream/`.
...
Clone the best candidate for each gap into `references/upstream/`:

```bash
GIT_TERMINAL_PROMPT=0 git clone --depth 1 <clone_url> references/upstream/<repo_name>
```

### Step A4: Verify each clone

For each cloned repo:
1. Read `README.md`: `cat references/upstream/<repo_name>/README.md`
...
1. Build `acquired_code_repos` dict from verified clones:
   ```json
   {
     "repo_name_1": "references/upstream/repo_name_1",
     "repo_name_2": "references/upstream/repo_name_2"
   }
   ```
...
2. Write `references/upstream/logs/repo_acquisition_agent.json`:
   ```json
   {
     "context_variables": {
...
some of them in my local machine, in the directory `references/upstream/`, use `ls`, `tree`,
...
### Step B2: Survey all repos in `references/upstream/`

Use Linux commands to navigate and read code:

| Action | Command |
|--------|---------|
| List repos | `ls references/upstream/` or `tree references/upstream/ -L 1` |
| View repo structure | `tree references/upstream/<repo>/ -L 3` |
| Find Python files | `find references/upstream/<repo>/ -name "*.py" -type f` |
| Read source file | `cat references/upstream/<repo>/model/attention.py` |
| Search across repos | `rg "class.*Attention" references/upstream/` or `grep -rn "sinkhorn" references/upstream/` |
| Read specific lines | `sed -n '100,200p' references/upstream/<repo>/file.py` |
...
Write `references/upstream/logs/code_survey_agent.json`:
...
## Tool mappings (reference -> Linux/Codex)
...
| `tracked_execute_command` (git clone) | `GIT_TERMINAL_PROMPT=0 git clone --depth 1 <url> references/upstream/<name>` |
...
- [ ] Best candidates cloned into `references/upstream/`
...
- [ ] `references/upstream/logs/repo_acquisition_agent.json` written

### Phase B (Code Survey)
- [ ] Code survey query built with `selected_idea` + `download_res` + `extra_repo_info`
- [ ] All repos in `references/upstream/` surveyed using `tree`, `cat`, `grep`, `find`
...
- [ ] `references/upstream/logs/code_survey_agent.json` written with complete `model_survey`
