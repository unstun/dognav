...
OBJECTIVE:
- Analyze codebases from reference papers in `references/upstream/`
...
|--------|---------|---------|
| Generate repo structure | `tree` | `tree references/upstream/repo/ -L 3` |
| List files | `ls`, `find` | `find references/upstream/repo/ -name "*.py" -type f` |
| Read a file | `cat`, `head`, `tail` | `cat references/upstream/repo/model/attention.py` |
| Read specific line range | `sed -n` | `sed -n '100,200p' references/upstream/repo/model/attention.py` |
| Search for text in files | `grep`, `rg` (ripgrep) | `rg "class.*Attention" references/upstream/repo/` |
| Search across all repos | `grep -r`, `rg` | `rg "sinkhorn" references/upstream/` |
| Count lines / get overview | `wc -l` | `wc -l references/upstream/repo/model/*.py` |
...
### Reference Codebases
- <repo_name>: references/upstream/<repo_name>/
