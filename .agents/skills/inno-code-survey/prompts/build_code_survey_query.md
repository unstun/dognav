...
I have carefully gone through these papers' github repositories and found download some of them in my local machine, in the directory `references/upstream/`, use `ls`, `tree`, and `find` to navigate the directory.
...
1. List all repos in `references/upstream/` using `tree` or `ls`
