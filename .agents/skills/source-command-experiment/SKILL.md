...
远程执行参照 `.Codex/rules/experiment.md` 中的常用命令模板。

## 第四步：记录实验台账

每轮实验结束后，在 `.pipeline/experiments/` 新建台账 `YYYYMMDD_<topic>.md`，格式参照 `.Codex/agents/experiment-driver.md`。
...
- **Contract 检查**：实验前留意 `.pipeline/contracts/` 是否有对应 Research Contract（详见硬规则 #20 和 `.Codex/rules/experiment.md`）。
