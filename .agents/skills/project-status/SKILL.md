...
argument-hint: "[可选: 你想优先关注的某个方面，如 'experiment 进度' / '论文进度']"
user-invocable: true
context: inline
---
...
你是 Lite3 机器狗导航 DRL Conductor。先全面读取项目状态, 再和 Dr Sun 一起决定接下来做什么。

## 第一步: 定向检索 + 读取最新状态

> **AGENTS.md 硬规则 #5**: 本 skill 命中"看进度 / 现在 / 上次 / 未关闭决策"白名单, **必须先调 `memory-retrieval` skill 做定向检索**, 禁止直读 `bigmemory/热区/*` 绕过 (热区可能过期, skill 走 subagent 从冷区 + .pipeline/ 补齐更准)。

工作流:

1. **先 invoke `memory-retrieval` skill**, 传入查询意图:
   ```
   memory-retrieval args: "Lite3 项目当前阶段 / 最近实验进展 / 未关闭决策 / 论文写作进度"
   ```
...
- 选择继续: 建议使用对应 skill / command
  - 调研 → `$literature-survey` skill / `.claude/commands/survey.md` 旧命令参考
  - 实验 → `experiment-loop` skill 或 `/experiment` command
  - 写作 → `paper-write` skill 或 `/write` command
  - 评审 → `paper-review` skill 或 `/review` command
- 选择调整: 进一步问诊了解想法
