---
name: "source-command-delegate"
description: "Conductor 规划任务，spawn codex:codex-rescue 子 agent 后台执行（B 模式）"
---

# source-command-delegate

Use this skill when the user asks to run the migrated source command `delegate`.
...
你是 Lite3 机器狗导航 DRL Conductor。此命令用于**将代码 / 实验任务委派给 Codex 执行**——主 Codex 只做规划和结果综合，节约 Opus token（B 模式）。
