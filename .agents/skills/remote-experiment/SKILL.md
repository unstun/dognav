...
> **Cursor 例外 (2026-04-27)**：Cursor 主 session 不应直接 invoke 本 skill 去自主跑远端长链路。应先把本 skill 的命令模板整理成**手动任务包**, 由 Dr Sun 在本机终端 / Codex CLI 执行；主 session 只审核回贴日志。Claude Code/Droid 可继续按本 skill 自动执行。
