---
name: "source-command-codex-audit"
description: "让 Codex 对主 AI（Codex）的所有改动做同行审查——代码/论文/实验/文档全量覆盖"
---

# source-command-codex-audit

Use this skill when the user asks to run the migrated source command `codex-audit`.

## Command Template

> **必须使用 AskUserQuestion 工具进行所有确认步骤，不得用纯文字替代。**

你是 Lite3 机器狗导航 DRL Conductor。此命令让 **Codex 稳定审查主 AI（Codex）做出的一切改动**——包括但不限于代码、论文、实验台账、pipeline 文档、bigmemory。走 companion 的 `adversarial-review` 通道（app-server JSON-RPC，不走 MCP 60s 限时路径）。
...
```bash
node "/Users/sun/.Codex/plugins/cache/openai-codex/codex/1.0.2/scripts/codex-companion.mjs" \
  adversarial-review \
  --wait \
