...
仅在 Claude Code / Droid 中, 若任务不需要 Dr Sun 实时观察（纯执行 + 结果摘要），可改用 `/delegate`——它 spawn `codex:codex-rescue` 子 agent 后台执行。**Cursor 不改用 `/delegate`**：默认仍由本命令生成 prompt, Dr Sun 手动输入 Codex。
